    ...
    public class MixCmsHelper
    {
            ...
            where TView : ViewModelBase<MixCmsContext, MixPost, TView>
        {
            ...
            try
            {
                ...
                Expression<Func<MixDatabaseValue, bool>> valPredicate = m => m.AttributeSetName == MixConstants.AttributeSetName.ADDITIONAL_FIELD_POST
        ...
        private static Expression<Func<MixDatabaseValue, bool>> GetValuePredicate(string fieldValue
            , MixEnums.CompareType filterType, MixEnums.MixDataType dataType)
        {
            ...
            switch (dataType)
            {
                ...
                case MixEnums.MixDataType.Time:
                    ...
                    {
                        valPredicate = FilterObjectSet<MixDatabaseValue, DateTime>("DateTimeValue", dtValue, filterType);
                    }
                    break;
                case MixEnums.MixDataType.Double:
                    if (double.TryParse(fieldValue, out double dbValue))
                    {
                        valPredicate = FilterObjectSet<MixDatabaseValue, double>("DoubleValue", dbValue, filterType);
                    }
                    break;
                case MixEnums.MixDataType.Boolean:
                    if (bool.TryParse(fieldValue, out bool boolValue))
                    {
                        valPredicate = FilterObjectSet<MixDatabaseValue, bool>("BooleanValue", boolValue, filterType);
                    }
                    break;
                case MixEnums.MixDataType.Integer:
                    if (int.TryParse(fieldValue, out int intValue))
                    {
                        valPredicate = FilterObjectSet<MixDatabaseValue, int>("IntegerValue", intValue, filterType);
                    }
                    break;
                ...
                default:
                    valPredicate = FilterObjectSet<MixDatabaseValue, string>("StringValue", fieldValue, filterType);
                    break;
            }
